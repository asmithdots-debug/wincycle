...
Независимый от затемнения переключатель в меню: подкладывает под тот же `Overlay` ещё один слой — `NSGlassEffectView` (Liquid Glass, публичный API AppKit, новый в macOS 26) — поверх или вместо `ShadeView`. Оба слоя живут в одном контейнере (`contentView` окна-подложки теперь обычный `NSView`, а `shade` и стекло — его сабвью), включаются и выключаются независимо друг от друга, каждый своим пунктом меню.

**Почему это отдельная, а не замена затемнению.** У `NSGlassEffectView` при пустом `tintColor` получается настоящее размытие содержимого позади окна — включая окна ДРУГИХ программ, не только своего приложения — без приватных вызовов. Это подтверждено вживую (см. коммит с скриншотами в PR): полноэкранная подложка с `NSGlassEffectView` размывает произвольное окно на заднем плане чисто, без зерна, которое раньше не удавалось убрать у `NSVisualEffectView` (см. ниже, раздел 6).

**Но регулировать силу размытия нельзя.** Полный список того, что публично отдаёт класс (вытащено через `class_copyPropertyList`/`class_copyMethodList`, а не по документации): `tintColor`, `cornerRadius`, `cornerConfiguration`, `clipsToBounds`, `contentView`, `style` — и всё. Ни радиуса, ни интенсивности размытия нет ни в публичном API, ни в доступных приватных проперти (`_variant`, `_subvariant` и подобные — это внутренние дискретные состояния адаптации, не непрерывный параметр). Само размытие полностью на стороне системы и не настраивается программой. Заодно проверено и отброшено:

- `style` имеет всего два значения, `.regular` и `.clear` — визуально почти неотличимы по силе размытия, разница в характере тонирования, не в интенсивности;
- `tintColor` ведёт себя нелинейно: рост alpha чёрного тона **увеличивает**, а не уменьшает яркость картинки под стеклом (измерено по шагам 0 → 0.95 — яркость монотонно растёт). Так что и «затемняющий» слайдер поверх `tintColor` не построить — именно поэтому стекло держит `tintColor = .clear` и не имеет собственной регулировки, только вкл/выкл.

**Требует macOS 26+.** Пункт меню оборачивается в `if #available(macOS 26.0, *)` и на более старых системах просто не показывается — `Info.plist` при этом не трогали (`LSMinimumSystemVersion` остаётся 13.0), затемнение по-прежнему работает везде, стекло — это чистая надстройка сверху.
