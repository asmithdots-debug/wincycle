...
- таймер на 50 мс — добирает переключение между окнами одной и той же программы, которое уведомление не ловит.
...
### Почему такт именно 50 мс

Такт был 0.3 секунды, и пользователь на живом использовании пожаловался на ощутимую задержку. Замеры показали, где она была на самом деле:

| что мерили | результат |
|---|---|
| один такт разбора списка окон (11 окон) | 0.6 мс — то есть такт дешёвый, дело не в нём |
| выдерживает ли фоновая программа заявленный интервал | 299–301 мс, App Nap таймер не душит |
| переключение **между программами**: от сигнала до подложки на месте | 9–36 мс — уже мгновенно |
| переключение **между окнами одной программы** при такте 0.3 с | в среднем 120 мс, в худшем случае 215 мс |

Вывод: путь через уведомление и так был мгновенным, вся задержка сидела в единственном месте — переключение внутри одной программы система не сообщает ничем, его ловит только опрос, поэтому такт и есть верхняя граница задержки. Лечится только уменьшением такта.

Цена (замерено по приросту процессорного времени):

| такт | нагрузка |
|---|---|
| пустой таймер 50 мс, без работы | 0.27% одного ядра |
| 50 мс с работой (как сейчас) | ~1.3–1.8% одного ядра |
| 100 мс с работой | 0.73% одного ядра |

Выбрано 50 мс: задержка внутри одной программы падает с 215 мс до 50 мс в худшем случае. Если когда-нибудь понадобится сэкономить батарею, самый очевидный размен — вернуть 100 мс, это ровно вдвое дешевле и, скорее всего, всё ещё незаметно на глаз.

### Уведомление приходит раньше, чем система перестраивает список окон

`didActivateApplicationNotification` опережает оконный сервер: в момент сигнала самым передним в `CGWindowListCopyWindowInfo` нередко всё ещё числится окно **старой** программы (воспроизводится не всегда, но регулярно — видно в замерах). Раньше код в этот момент сразу делал `updateDimming(force: true)`, то есть ставил подложку под старое окно — на мгновение темнело ровно то окно, на которое переключились, и разъезжалось это только следующим тактом.

Поэтому `appActivated` больше не верит списку сразу: `followActivation(of:until:)` перепроверяет каждые 8 мс, пока переднее окно не окажется окном той программы, о которой пришло уведомление. Предел 0.4 с — на случай, когда переднего окна у программы так и не появится (активировали программу вообще без окон), тогда просто обновляемся по тому, что есть.

### Что делает такт дешёвым

- `realWindows()` останавливается на **двух** подходящих окнах: наружу нужно только самое переднее окно и признак «оно тут не одно».
- Список окон разбирается лениво, как `NSArray`/`NSDictionary`. Приведение `as? [[String: Any]]` переводит в свифтовые типы весь массив разом, включая окна, до которых мы не дойдём — поэлементный разбор дешевле втрое (0.35 мс → 0.11 мс, замерено).
- Всё, что дёргается на каждом такте, защищено проверкой «уже так и есть»: `Overlay.apply(alpha:)`, `Overlay.setGlass(enabled:)`, `showBorder` через `lastBorderFrame`.

Кэшировать `NSRunningApplication(processIdentifier:)` **не нужно** — проверено, поиск стоит 0.002 мс, это шум на фоне всего остального.

### `orderFrontRegardless`, а не `orderFront`

Подложки поднимаются через `orderFrontRegardless()`: обычный `orderFront` у программы, которая не активна (а WinCycle не активна никогда — она `.accessory`), AppKit вправе отложить до её активации. На замерах разницы в задержке не видно, но полагаться на это неправильно.
...
**Смена активного окна на лету.** При переключении между окнами (например, кликом) подложка двигается сразу по уведомлению, не дожидаясь следующего такта опроса — иначе затемнённым на целый такт оказывалось бы как раз то окно, на которое переключились. Про то, почему уведомлению нельзя верить сразу же, — раздел 2.
