...
**Заливка — виньетка, не плоский цвет.** `ShadeView.draw()` рисует радиальный градиент (`CGGradient`/`drawRadialGradient`): прозрачно в центре экрана, темнее к краям, максимум — тот же `dimAlpha`, что задаёт ползунок. Раньше была плоская заливка одним цветом на весь экран; виньетка мягче и не выглядит как «весь экран одним оттенком».
