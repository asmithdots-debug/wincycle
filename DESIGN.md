...
## 3. Раскладка: фиксированная сетка

Не рекурсивная спираль (в ней раньше была своя, отброшенная версия — см. раздел 11), а фиксированная сетка до восьми ячеек, целиком выписанная как таблица случаев `gridRects(count:in:)`: по одному правилу деления на каждое n от 2 до 8, без общего рекурсивного алгоритма — самих правил всего семь, проще выписать прямо.

Первое окно — весь экран. Второе делит его пополам, новое встаёт справа. Третье делит правую половину на верхнюю и нижнюю четверти (новое — снизу), четвёртое — то же с левой половиной: получаются четыре равные четверти. Пятое—восьмое, по часовой стрелке начиная с верхней правой четверти, делят каждую следующую четверть пополам ещё раз (новое — во второй половине). Какая ячейка кому достанется, зависит **только от того, каким по счёту окно открыто на этом экране** — не от того, какое окно было активно в момент открытия. Внутри каждого деления окно, уже стоявшее в делящейся ячейке, остаётся в первой (левой либо верхней) половине, а новое — во второй: то же правило, что и у самого первого деления экрана пополам.

```
   1 окно            2 окна             3 окна             4 окна
┌───────────┐   ┌─────┬─────┐     ┌─────┬─────┐     ┌─────┬─────┐
│           │   │     │     │     │     │  2  │     │  1  │  2  │
│     1     │   │  1  │  2  │     │  1  ├─────┤     ├─────┼─────┤
│           │   │     │     │     │     │  3  │     │  4  │  3  │
...
Между окнами и по краю экрана — зазор 8pt, ровно такой же, какой даёт Raycast Window Management (измерено вживую). Внешний зазор — усечение видимой области экрана перед делением, внутренний вставлен прямо в точку каждого деления.

### Предел в восемь ячеек

Дальше делить нечего — девятая долька на экране ноутбука уже нечитаема. Поэтому ячеек максимум восемь (`maxGridTiles`), а девятое и последующие окна в сетку вообще не попадают: такое окно сразу растягивается на весь экран (`stretchBeyondCapacity`) — тем же движением, что и ручное растягивание, — и относится к `floated`. Восемь окон сетки при этом не трогаются вовсе.

Раньше здесь была стопка — шестое и последующие окна складывались друг на друга в последней ячейке (см. раздел 11, там же почему от неё отказались). Сетка на восемь ячеек и авто-растягивание сверх неё проще и предсказуемее: ни одна ячейка не становится незаметно уже трёхсот точек, а окна за пределами сетки ведут себя ровно так же, как окно, растянутое вручную, — то есть по уже отлаженному пути `floated`, не по отдельному новому.
...
Порядок при этом восстанавливается **по геометрии, а не по слоям**, приблизительно: сортировка «от большего окна к меньшему» в среднем воспроизводит исходный порядок открытия (в сетке ячейки открытых раньше окон в среднем не меньше ячеек открытых позже), но точного восстановления не гарантирует — при полной сетке несколько ячеек равны между собой. Это не страшно: следующее же открытие или закрытие окна пересчитает раскладку заново, уже с реальным порядком. Равные по площади ячейки разводятся по положению, сверху вниз и слева направо.
...
**Номер ячейки брать ДО изъятия окна из порядка.** Иначе при движении вперёд по сетке изъятие сдвигает цель на шаг назад, и окно вставляется ровно туда, откуда его взяли.

### Перестановка — это каскад, а не обмен

Окно вынимается из своего места в порядке и вставляется на место цели. Всё, что было после точки вставки, сдвигается по сетке на шаг — как при обычном открытии нового окна. Это не изолированный обмен двух окон местами.
...
1. рост числа окон 1 → 8 → 9, по одному пересчёту на окно, сверить координаты каждого шага с ожидаемой ячейкой;
2. закрытие окна из середины сетки — остальные должны пересчитаться по новой сетке для уменьшившегося числа окон;
3. перестановка клавишами **в обе стороны** по сетке;
...
Первая версия раскладки, работавшая до текущей фиксированной сетки (раздел 3). Первое окно занимало экран целиком, каждое следующее делило пополам ячейку **последнего добавленного** окна, направление деления выбиралось по пропорциям самой ячейки (широкая — вертикально, высокая — горизонтально). Получалась спираль: одно большое окно и всё более мелкие рядом с ним, вглубь до пяти ячеек, а всё сверх — стопкой в последней.

Отказались не из-за отдельного бага, а потому что сама схема давала непредсказуемое место для новой ячейки: какое именно окно «последнее добавленное» — а значит, чья ячейка сейчас будет делиться, — зависело от того, какое окно было активно в момент открытия нового, то есть от истории переключений, а не только от того, сколько окон уже открыто. Новое окно почти всегда вставало «где-то рядом» с активным, но не в предсказуемом заранее месте на экране. Фиксированная сетка избавляет от этой зависимости совсем: место окна определяется только тем, каким по счёту оно открыто — при том же числе окон оно всегда одно и то же, независимо от того, что было активно.

Заодно ушла и стопка (раздел 3, «Предел в восемь ячеек») — вместе с ней исчезли три места, которые из-за неё усложнялись специальной обработкой совпадающих рамок (раздел 8, «Про новую сущность, ломающую старые допущения»): сопоставление с Accessibility, распознавание ручных перестановок, круговая сортировка перебора. Устройство самой сетки: раздел 3.
