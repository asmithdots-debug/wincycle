...
### Неровные деления и свободные ячейки

Первая версия дерева считала перестановку исключительно через «на чьё место встало окно» (раздел выше). Это работает только тогда, когда целевое место кто-то ДО этого уже занимал. У этого есть дыра: если окно — единственный хозяин большой области (например, всего два окна на экране, каждое на свою половину) и его ужимают до четверти этой же половины, сравнивать не с кем — вторую четверть никто раньше не занимал. Раскладка молчала, а освободившийся кусок навсегда оставался дырой: ни новое окно, ни следующая перестановка её не подхватывали.

Решение — две вещи разом:

**Разрезы стали неровными.** `TileNode.split` хранит `ratio` — долю площади первого потомка (0.5 у обычного деления пополам, как раньше). Когда окно ужимается внутри своей же ячейки, `shrinkSplit` смотрит, какое измерение не поменялось (ширина или высота), к какому краю окно прижато, и вычисляет РЕАЛЬНУЮ пропорцию, которую выбрал человек, — не округляя её до половины.

**Пустой лист — legitimate состояние, а не отсутствие листа.** `leaf([])` значит «место есть, просто сейчас там никто не стоит». Освободившийся при `shrinkSplit` кусок становится именно таким листом — его может занять следующее новое окно (`insertNew` теперь сначала ищет свободный лист, и только если такого нет — делит активную/хвостовую ячейку) или следующая перестановка (`mostOverlappedEmptyLeaf` — та же логика перекрытия ≥30%, что и `mostOverlapped`, только цель не окно, а уже существующая пустая ячейка).

```
было: A занимает всю область один          A ужалось до четверти:
┌───────────┐                              ┌─────┬─────┐
│           │                              │  A  │  •  │   • — свободный
│     A     │        →                     │     │     │      лист, ratio
│           │                              │     │     │      посчитан
...
следующее новое окно B — не в хвост спирали, а прямо в свободную ячейку:
┌─────┬─────┐
│  A  │  B  │
│     │     │
└─────┴─────┘
...
Свободный лист **не считается** в лимит `maxTiles` (`leafCount` пропускает пустые) — он не занимает видимого места сам по себе. Из-за этого возможна ситуация, когда РЕАЛЬНЫХ окон на экране одновременно больше пяти без стопки: если человек сам последовательно ужимает окна, отвоёвывая место под новые, каждое такое деление — его собственное явное решение, а не автоматический рост, который лимит и должен был сдерживать. Ограничивать это не стали — деление 50/50 при обычном открытии окна лимит по-прежнему соблюдает строго.

Порядок проверки при обнаруженном дрейфе теперь такой: сначала «встал на место другого окна» (`mostOverlapped`), затем «занял существующую свободную ячейку» (`mostOverlappedEmptyLeaf`), и только если ни то ни другое не подошло — «ужался внутри своей же ячейки» (`shrinkSplit`). Если не подходит ни один из трёх — раскладка, как и раньше, ничего не делает: угадывать по-настоящему произвольное перемещение (не связанное ни с чьей чужой территорией, ни со своей прежней) смысла нет.
...
**Обмен местами по клавишам.** Option+Command+Tab меняет активное окно местами со следующим по тому же кругу, что и обычный перебор; Shift — с предыдущим. Это единственная команда, которая делает НАСТОЯЩИЙ обмен (`swapWindows`) — просто меняет содержимое двух листьев местами, — в отличие от перестановки, распознанной по перекрытию (раздел 6), которая специально каскадит, а не меняет местами. Разница оправдана тем, кто именно просит: здесь пользователь явно называет два конкретных окна и просит поменять их местами — «поменять местами» и должно значить ровно это, без побочных эффектов на соседей. Если одно из окон стоит в стопке, меняется только оно — остальные окна той же стопки не трогаются.
...
### Про то, что детектору нужна жертва

**Распознавание перестановки работало только «на чьё место встали».** Хватало на любую заполненную раскладку (проверено много раз), но ломалось ровно там, где окон меньше, чем ячеек, до которых хочет добраться человек, — например, два окна на весь экран, и одно ужимают до четверти своей же половины. Вторую четверть раньше никто не занимал, сравнивать не с кем — раскладка молчала, и освободившийся кусок навсегда оставался дырой. Обнаружено не гаданием, а живым отчётом пользователя («не могу окно в область отправить») и подтверждено при попытке воспроизвести на реальном рабочем столе.

> **Урок.** Механизм, работающий «по совпадению с существующим», незаметно предполагает, что нужное всегда УЖЕ есть чем-то занято. Стоит явно спросить: а что если целевое место пока ничьё? Здесь ответом стало третье состояние листа («свободна, но не отсутствует») — конкретно для этого случая, а не общий принцип на будущее.
...
8. обмен местами Option+Command+Tab — фокус остаётся на переставленном окне;
9. окно ужимается внутри своей же ячейки (например, было одно на всю половину экрана, стало занимать только четверть) — освободившийся кусок должен достаться следующему новому окну ИЛИ следующей перестановке, без дыр;
10. расход процессора в покое.

При проверке пункта 9 (и вообще ручных перестановок) на реальном рабочем столе: любое постороннее окно, которое в этот момент само меняется (пользователь параллельно с чем-то работает), может сбить один конкретный тест — раскладка отреагирует на НЕГО, а не на то, что проверяется. Верный признак — журнал показывает пересчёт для другого владельца окна, а не для того, что двигали. Решение простое: изолировать проверку одноразовыми окнами (TextEdit) и не полагаться на реальные окна пользователя, если они не единственные на экране.
